...
const waitForService = async (
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    if (available) {
      logger.info(
        `✅ ${serviceName} is ready (attempt ${i + 1}/${maxRetries})`,
      );
      return true;
    }
    ...
    if (i < maxRetries - 1) {
      logger.info(
        `⏳ Waiting for ${serviceName} to be ready... (attempt ${i + 1}/${maxRetries})`,
      );
  ...
  try {
    ...
    if (appointmentServiceAvailable) {
      ...
      if (appointmentSchema) {
        ...
        logger.info(
          "✅ Appointment service GraphQL schema added to federation",
        );
      }
    } else {
    ...
    logger.info(
      `✅ GraphQL federation complete with ${schemas.length} service(s)`,
    );

    // Update the connected-services gauge for Prometheus monitoring
    try {
      const { connectedServices } = require("../telemetry/metrics");
      connectedServices.set(schemas.length);
    } catch {
      /* metrics not yet initialised — safe to skip */
    }
