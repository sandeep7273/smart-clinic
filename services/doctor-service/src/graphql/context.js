...
const createContext = async ({ req }) => {
  const context = {
    ...
    correlationId: req.headers['x-correlation-id'] ||
      ...
      `doctor-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  if (token) {
    ...
    if (gatewayUserId) {
      ...
      try {
        ...
        // Validation failed; context.token is still set so callers that only
        // check token presence will proceed (resolvers that need context.user
        // will enforce role checks themselves).
      }
    }
  }
