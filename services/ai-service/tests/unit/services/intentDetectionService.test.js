...
describe("IntentDetectionService", () => {
  ...
  describe("detectIntent", () => {
      ...
      expect(mockGroqInstance.chat.completions.create).toHaveBeenCalledWith(
        expect.objectContaining({
          messages: expect.arrayContaining([
            ...
            expect.objectContaining({
              role: "assistant",
              content: "You might need an orthopedist",
            }),
          ]),
        }),
      );
    ...
    it("should detect generic plural doctor search without the LLM", async () => {
      const result =
        await intentDetectionService.detectIntent("Search for doctors");
  ...
  describe("extractSpecialization", () => {
    ...
    it("should not match specialization keywords inside unrelated words", () => {
      const result =
        intentDetectionService.extractSpecialization("Search for doctors");
      expect(result).toBe(null);
    });
  ...
  describe("normalizeSpecialization", () => {
    it("should normalize specialist role to department", () => {
      expect(
        intentDetectionService.normalizeSpecialization("cardiologist"),
      ).toBe("Cardiology");
      expect(
        intentDetectionService.normalizeSpecialization("dermatologist"),
      ).toBe("Dermatology");
      expect(
        intentDetectionService.normalizeSpecialization("pediatrician"),
      ).toBe("Pediatrics");
      expect(
        intentDetectionService.normalizeSpecialization("neurologist"),
      ).toBe("Neurology");
    });

    it("should handle department names directly", () => {
      expect(intentDetectionService.normalizeSpecialization("Cardiology")).toBe(
        "Cardiology",
      );
      expect(intentDetectionService.normalizeSpecialization("cardiology")).toBe(
        "Cardiology",
    ...
    it("should map keywords to departments", () => {
      expect(intentDetectionService.normalizeSpecialization("heart")).toBe(
        "Cardiology",
      );
      expect(intentDetectionService.normalizeSpecialization("skin")).toBe(
        "Dermatology",
      );
      expect(intentDetectionService.normalizeSpecialization("bone")).toBe(
        "Orthopedics",
      );
      expect(intentDetectionService.normalizeSpecialization("brain")).toBe(
        "Neurology",
      );
    });
    ...
    it("should handle ENT variations", () => {
      ...
      expect(intentDetectionService.normalizeSpecialization("nose")).toBe(
        "ENT",
      );
      expect(intentDetectionService.normalizeSpecialization("throat")).toBe(
        "ENT",
      );
    });
    ...
    it("should handle partial matches", () => {
      expect(
        intentDetectionService.normalizeSpecialization("cardiac surgery"),
      ).toBe("Cardiology");
      expect(
        intentDetectionService.normalizeSpecialization("pediatric care"),
      ).toBe("Pediatrics");
    });
  });
  ...
  describe("INTENTS constants", () => {
    it("should have all required intent types", () => {
      ...
      expect(intentDetectionService.INTENTS).toHaveProperty(
        "SHOW_APPOINTMENTS",
      );
